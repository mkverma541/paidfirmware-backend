...
async function processBatch(connection, offset, limit) {
    ...
    for (const file of files) {
        let baseSlug = file.slug || slugify(file.title || '', {
            lower: true,
            replacement: '-',
            remove: /[*+~.()'"!:@]/g
        });
        ...
        const uniqueSlug = await getUniqueSlug(connection, baseSlug, file.file_id);
...
async function getUniqueSlug(connection, baseSlug, fileId) {
