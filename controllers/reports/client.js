...
const downloadClientReportExcel = async (req, res) => {
  const { client_id, start_date, end_date } =
    req.query;
  ...
  try {
    ...
    const worksheet = workbook.addWorksheet("Client Projects");
    ...
    const fileName = `Client_Projects_${dateRange}.xlsx`;

    // 🎯 Add Project Summary Header
    worksheet.mergeCells("A1:H1");
    worksheet.getCell("A1").value = `Project Summary Report - Client`;
    ...
    worksheet.getCell(
      "A2"
    ).value = `Client Name: ${client.client_name} (Client Code: ${client.client_code})`;
